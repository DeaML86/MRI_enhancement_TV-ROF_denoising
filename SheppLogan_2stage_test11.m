...
mu=1;
kMax=25;
MatchType={'None', 'ExactColtuc', 'UniformHistMatch', 'ExactColtuc', 'UniformHistMatch'};
Str= {'SplitB TV Std','SbTV Exact Noise', 'SbTV Approx Noise',  'SbTV Exact True', 'SbTV Approx True'};
nOfTests=size(MatchType,2);


%for s_noise=0.0045:0.005:0.02
%s=s_noise;
%X_noise = ricernd(X_true, s); %add rician noise

for sigma=0.01:0.03:0.11
[n,m] = size(X_true);
s=sigma;
...
subplot(1,2,1); imagesc(X_true); axis off image; title('Original'); zoom(2);
subplot(1,2,2); imagesc(X_noise); axis off image; title('Noisy'); zoom(2);
%montage({imagesc(X_true),imagesc(X_noise)})
...
for i=1:nOfTests
    subplot(2,3,i); imagesc(u{i}); axis image off; title(string(Str{i})); zoom(2);
end
tightfig;
